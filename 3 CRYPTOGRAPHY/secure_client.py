...
def TLS_handshake_client(connection, server_ip=SERVER_IP, server_port=SERVER_PORT):
    ...
    s.sendall(bytes(encode_message("I would like a TLS Handshake, please!"), 'utf-8'))

    #  receive signed certificate from the server
    signed_certificate = connection.recv(1024).decode('utf-8')

    #  verify certificate with certificate authority's public key    
    cryptgraphy_simulator.verify_certificate(CA_public_key, signed_certificate)

    #  extract server's public key, IP address, and port from the certificate
    server_public_key = signed_certificate[signed_certificate.index('~port~')+6:-1]

    #  generate a symmetric key to send to the server
    symmetric_key = cryptgraphy_simulator.generate_symmetric_key()

    #  use server's public key to encrypt the symmetric key
